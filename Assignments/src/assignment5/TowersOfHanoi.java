...
public class TowersOfHanoi
{
	...
	{
		//If the 'from' tower is the same as the 'to' tower
		if(from == to)
			throw new Exception("Abort! Illegal tower request!");


		MyStack<Integer> start = null;//Temporary start tower
		MyStack<Integer> finish = null;//Temporary end tower
		...
		case 'C':
			...
			break;
		default:
			throw new Exception("Abort! Illegal tower ID!");//If the tower does not exist.
		}
		...
		if(start.size() < 1)
			throw new Exception("Abort! Illegal tower request!");//If the start tower is empty

		int s = start.peek();
		int f = 0;
		try{
			f = finish.peek();
		} catch(NoSuchElementException e){//If the 'to' tower is empty then guarantees f to be less than any value possible so that there is no exception
			f = 0;
		}
		if(s > f && finish.size() != 0)
			throw new Exception("Abort! Illegal disc movement!");//If the item in 'from' is larger than 'to' AND 'to' is not empty

		int temp = start.pop();//the process of moving the item from the beginning to end.
		finish.push(temp);
		numOfMoves++;
